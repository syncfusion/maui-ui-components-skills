...
## AI-Powered Search Issues

### Azure OpenAI Connection Failures

**Problem:** AI search not working or throwing exceptions.

**Solutions:**

1. **Verify credentials:**
   ```csharp
   internal const string Endpoint = "https://your-resource.openai.azure.com/";
   internal const string Key = "your-api-key";
   internal const string DeploymentName = "your-deployment";
   ```
...
4. **Implement timeout:**
   ...
   var response = await Client.CompleteAsync(prompt, cts.Token);
   ```

### Prompt Not Returning Expected Results

**Problem:** AI returns incorrect or empty results.
...
1. **Improve prompt specificity:**
   ```csharp
   var prompt = $@"
       Filter items from this list: {itemsList}
       User input: {userInput}
       
       Rules:
       - Return only items from the list
       - One item per line
...
2. **Parse response carefully:**
   ```csharp
   var filteredNames = completion
       .Split('\n')
       .Select(x => x.Trim())
       .Where(x => !string.IsNullOrEmpty(x))
       .Where(x => !x.Contains("Here are"))  // Filter out explanations
       .ToList();
   ```
